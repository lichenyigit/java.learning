...
public class HashAlgorithmPercentTest {
	...
	public static void main(String[] args) {
		...
		List<Node> allNodes = generateNodes(NODE_COUNT);
		System.out.println("Normal case : nodes count : " + allNodes.size());
		call(allNodes, map);
		
		allNodes = generateNodes(NODE_COUNT + 8);
		System.out.println("Added case : nodes count : " + allNodes.size());
		call(allNodes, map);
		
		allNodes = generateNodes(NODE_COUNT - 10);
	 ...
	 * 		the node list
	 */
	private static List<Node> generateNodes(int nodeCount) {
